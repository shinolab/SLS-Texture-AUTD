...
LastEditTime: 2024-04-10 17:26:28
...
class VideoThread(QThread):
   ...
   def __init__(self):
       ...
       self.positions = []  # previous coordiantes
       self.timestamps = [] #corresponding time
   ...
   def run(self):
       ...
       while self._run_flag:
           ...
           #use official functions to obtain spatial information
           ...
           # self.change_pixmap_signal.emit(color_img)
          
           # using temporal difference
          
           #if prev_depth_img is not None:
           #    # diff calculation
           #    depth_img_8u = cv2.convertScaleAbs(depth_img, alpha = (255/65535.0))
           #    prev_depth_img_8u = cv2.convertScaleAbs(prev_depth_img, alpha = (255/65535.0))
           #    diff_img = cv2.absdiff(depth_img_8u, prev_depth_img_8u)
           #    # binarization
           ...
           #        cv2.circle(color_img, (cent_x, cent_y), 5, (255, 255, 255), -1)
           #        height = depth_img[cent_y, cent_x]
           #        #use official functions to obtain spatial information
           #        intrinsics = depth_frame.profile.as_video_stream_profile().intrinsics
           #        point = rs.rs2_deproject_pixel_to_point(intrinsics,[cent_x, cent_y],height)
           #        x_dis, y_dis, height = point
